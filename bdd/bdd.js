...
const getLuncStaking = async(limit, timeunit) => {
    const rqt = `SELECT * FROM tblLuncStaking WHERE b${timeunit}=TRUE ORDER BY enregNumber DESC LIMIT ${limit}`;
    return await myquery(rqt);
}
...
const getCommunityPoolContent = async(limit, timeunit) => {
    const rqt = `SELECT * FROM tblCommunityPoolContent WHERE b${timeunit}=TRUE ORDER BY enregNumber DESC LIMIT ${limit}`;
    return await myquery(rqt);
}
...
const getOraclePoolContent = async(limit, timeunit) => {
    const rqt = `SELECT * FROM tblOraclePoolContent WHERE b${timeunit}=TRUE ORDER BY enregNumber DESC LIMIT ${limit}`;
    return await myquery(rqt);
}
